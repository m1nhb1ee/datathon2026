...
def chart3_promo_roi(oat):
    ...
    for label, frame, color, dy in [
        ("KEEP", promo.nlargest(3, "total_net_contribution"), PALETTE["positive"], 8),
        ("CUT", promo.nsmallest(3, "total_net_contribution"), PALETTE["negative"], -28),
    ]:
        for _, r in frame.iterrows():
            ax.annotate(
                ...
                color=color, bbox=dict(boxstyle="round,pad=0.3", fc="white", ec=color, alpha=0.9),
            )
...
def chart5_scenario_simulation(oat):
    ...
    for ax, col, fmt in [
        ...
        for i, v in enumerate(vals):
            ...
            ax.text(i, v + span * 0.025 if v >= 0 else v - span * 0.08,
...
def chart7_triage(oat, overlap_events):
    ...
    ax.xaxis.set_major_formatter(mticker.FuncFormatter(money_m))
    ax.set_title("Promo Triage - keep, cut, or reschedule by GP contribution",
                 fontsize=15, fontweight="bold", pad=12)
    ...
    for _, r in plot_df.tail(5).iterrows():
        ax.text(r["total_net_contribution"], r["promo_name"], " KEEP", va="center",
                ha="left", color=PALETTE["positive"], fontsize=8, fontweight="bold")
    ax.annotate(
